      ...
      <div className="relative overflow-hidden" style={{ background: "radial-gradient(ellipse at 35% 25%, rgba(80,50,140,0.12), transparent 50%), radial-gradient(ellipse at 70% 70%, rgba(30,60,120,0.08), transparent 40%), #0a0a0e" }}>
        <svg ref={svgRef} viewBox={`0 0 ${W} ${H}`} className="w-full h-full select-none touch-none block absolute inset-0"
          ...
          <defs>
            ...
            <radialGradient id="sph-hl" cx="30%" cy="22%" r="30%"><stop offset="0%" stopColor="white" stopOpacity="0.4" /><stop offset="100%" stopColor="white" stopOpacity="0" /></radialGradient>
          ...
          <g ref={rootRef}>
            ...
            {nodes.map(n => {
              ...
              return <g key={n.id} className="nd" transform={`translate(${n.x},${n.y})`} style={{ cursor: "pointer" }}
                ...
                {n.kind === "salary" && <><circle r={n.r} fill="url(#sph-salary)" /><circle r={n.r} fill="url(#sph-hl)" /><g clipPath="url(#clip-sal)">{Array.from({ length: 16 }, (_, i) => <line key={i} x1={-n.r + 4 + i * 4} y1={n.r - 1} x2={-n.r + 4 + i * 4 + (i % 2 ? 1 : -1)} y2={n.r - 1 - (3 + (i * 7 % 6))} stroke="#2dd45a" strokeWidth={1.2} strokeLinecap="round" opacity={0.5 + (i % 3) * 0.15} />)}</g><text y={-5} textAnchor="middle" fontSize={11} fontWeight={600} fill="#fff" style={ts}>Salaire</text><text y={9} textAnchor="middle" fontSize={9} fill="rgba(255,255,255,0.75)">{formatMoney(salary)}/mois</text></>}

                {n.kind === "expenses" && <>{isOverBudget && <circle r={n.r + 16} fill="url(#glow-lava)" />}{isOverBudget && <circle r={n.r + 6} fill="none" stroke="#ff4500" strokeOpacity={0.3} strokeDasharray="2 3" />}<circle r={n.r} fill={isOverBudget ? "url(#sph-lava)" : "url(#sph-expenses)"} />{isOverBudget && <circle r={n.r} fill="url(#sph-lava)" filter="url(#turb-lava)" opacity={0.5} />}<circle r={n.r} fill="url(#sph-hl)" /><text y={-5} textAnchor="middle" fontSize={10} fontWeight={600} fill="#fff" style={ts}>Dépenses</text><text y={9} textAnchor="middle" fontSize={9} fill={isOverBudget ? "#ffaa70" : "rgba(255,255,255,0.7)"}>{totalExpenseFlows > 0 ? formatMoney(totalExpenseFlows) : "0 €"}/m</text>{isOverBudget && <text y={22} textAnchor="middle" fontSize={8} fill="#ff6b35" fontWeight={600}>ALERTE</text>}</>}
                ...
                {n.kind === "portfolio" && <><circle r={n.r + 4} fill="none" stroke={n.color} strokeOpacity={0.06} /><circle r={n.r} fill={`url(#sph-${n.id})`} stroke={n.color} strokeOpacity={isExp ? 0.4 : 0.12} strokeWidth={isExp ? 1.5 : 0.5} /><circle r={n.r} fill="url(#sph-hl)" /><text y={-6} textAnchor="middle" fontSize={11} fontWeight={600} fill="#fff" style={ts}>{n.label}</text><text y={9} textAnchor="middle" fontSize={9} fill="rgba(255,255,255,0.7)">{n.sub}</text>{(n.gainVal ?? 0) !== 0 && <text y={22} textAnchor="middle" fontSize={8} fill={(n.gainVal ?? 0) >= 0 ? "#34d399" : "#fb7185"}>{(n.gainVal ?? 0) >= 0 ? "+" : ""}{formatMoney(n.gainVal ?? 0)}</text>}</>}

                {n.kind === "goal" && <><circle r={n.r + 3} fill="none" stroke={n.color} strokeOpacity={0.12} strokeDasharray="3 4" /><circle r={n.r} fill={`url(#sph-${n.id})`} />{(n.color === "#fb923c" || n.color === "#fbbf24") && <><clipPath id={`clip-${n.id}`}><circle r={n.r} /></clipPath><g clipPath={`url(#clip-${n.id})`}><ellipse cx={0} cy={n.r * 0.55} rx={n.r * 0.9} ry={5} fill="#f5d280" opacity={0.35} /><ellipse cx={0} cy={n.r * 0.7} rx={n.r} ry={4} fill="#2898d4" opacity={0.2} filter="url(#turb-water)" /></g></>}<circle r={n.r} fill="url(#sph-hl)" />{(gp ?? 0) >= 1 && <circle r={n.r + 6} fill="none" stroke="#34d399" strokeOpacity={0.45} strokeWidth={1.5} />}<text y={-4} textAnchor="middle" fontSize={10} fontWeight={600} fill="#fff" style={ts}>{n.label.length > 12 ? n.label.slice(0, 11) + "…" : n.label}</text><text y={10} textAnchor="middle" fontSize={9} fill="rgba(255,255,255,0.7)">{n.sub}</text></>}
